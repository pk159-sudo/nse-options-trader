...
function OIStrikeRowItem({
  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-2.5 rounded-md t-bg-muted t-border-sub border-l-2 hover:t-bg-hover transition-colors">
      <span className="text-xs font-mono font-bold t-text-2">{row.strike}</span>
      <span className={`text-xs font-mono font-bold ${changeColor}`}>
        {isPositive ? "+" : ""}
        {formatPlain(row.oiChange)}
      </span>
      <span className="text-xs font-mono t-text-4">{formatPlain(row.oiCurrent)}</span>
    </div>
  );
}
    ...
    <Card className="t-bg-card t-border-main overflow-hidden">
      ...
      <CardContent className="p-3 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] t-bg-hover t-text-4 px-2 py-0.5 rounded-full font-bold">
            {data.length} strike{data.length !== 1 ? "s" : ""}
          </span>
        </div>

        <div className="text-[10px] t-text-5 font-mono flex justify-between px-1">
          <span>Strike</span>
          <span>Delta OI</span>
          <span>OI</span>
        </div>
        ...
        <div className="space-y-1.5">
          {data.length === 0 ? (
            <p className="text-xs t-text-6 italic text-center py-3">
...
export function OISummaryPanel() {
  ...
  if (!oiSummary) {
    return (
      <div className="space-y-4">
        <div className="t-bg-card t-border-main border rounded-xl p-4 flex items-center gap-3">
          <BarChart3 className="h-5 w-5 t-text-6" />
          <div>
            <h3 className="text-sm font-bold t-text-3">OI Summary (Delta)</h3>
            <p className="text-xs t-text-5 mt-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <Card key={card.title} className="t-bg-card t-border-main opacity-50">
              <div className={`h-1 ${card.border}`} />
              <CardContent className="p-3 text-center">
                <p className="text-xs t-text-6 italic">
                  {card.title} - Waiting...
                </p>
              </CardContent>
            </Card>
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="t-bg-card t-border-main border rounded-xl p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2 t-bg-hover rounded-lg">
              <BarChart3 className="h-5 w-5 text-emerald-400" />
            </div>
            <div>
              <h3 className="text-sm font-bold t-text-2">OI Summary (Delta)</h3>
              <p className="text-[11px] t-text-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
          <div className="t-bg-muted rounded-lg p-2.5 text-center">
            <p className="text-[10px] t-text-5 uppercase">ATM Strike</p>
            <p className="text-sm font-bold text-amber-400 mt-0.5">{oiSummary.atmStrike}</p>
          </div>
          <div className="t-bg-muted rounded-lg p-2.5 text-center">
            <p className="text-[10px] t-text-5 uppercase">Range</p>
            <p className="text-sm font-bold t-text-3 mt-0.5">{oiSummary.dataRange}</p>
          </div>
          <div className="t-bg-muted rounded-lg p-2.5 text-center">
            <p className="text-[10px] t-text-5 uppercase">Total CE OI Chg</p>
            <p className={`text-sm font-bold mt-0.5 ${oiSummary.ceTotalChange > 0 ? "text-emerald-400" : oiSummary.ceTotalChange < 0 ? "text-red-400" : "t-text-4"}`}>
              {oiSummary.ceTotalChange > 0 ? "+" : ""}
              {formatPlain(oiSummary.ceTotalChange)}
            </p>
          </div>
          <div className="t-bg-muted rounded-lg p-2.5 text-center">
            <p className="text-[10px] t-text-5 uppercase">Total PE OI Chg</p>
            <p className={`text-sm font-bold mt-0.5 ${oiSummary.peTotalChange > 0 ? "text-emerald-400" : oiSummary.peTotalChange < 0 ? "text-red-400" : "t-text-4"}`}>
